# 📸 Media Assets

## Overview

This directory contains visual assets for the portfolio showcase.

---

## Banner Image

**File:** `banner.png`

The banner image is displayed at the top of the main README and showcases the platform's monitoring capabilities through a composition of dashboard elements:
- Portfolio performance metrics
- Risk monitoring displays
- Strategy tracking
- Real-time state visualization

**Dimensions:** 1584 x 396 pixels (4:1 aspect ratio)  
**Format:** PNG with transparency

---

## Screenshots

**Location:** `screenshots/`

**Available screenshots:**
- Dashboard overview (portfolio state and strategy activity)
- Strategy detail (strategy-level monitoring and position tracking)
- Risk dashboard (portfolio risk aggregation and limit monitoring)
- Portfolio analytics (performance attribution and aggregation)

**Note:** Screenshots show system output with sample data

---

## Videos

**Location:** `videos/`

**Planned videos:**
- System demonstration (execution flow and monitoring)
- Architecture walkthrough (C++ components and messaging)
- Build and compilation process

**Format:** MP4, unlisted YouTube links

---

## Adding Media

When adding media assets:

1. **Optimize file sizes** - Compress images, use appropriate resolutions
2. **Remove sensitive data** - No real account info, API keys, etc.
3. **Use descriptive names** - `dashboard-overview.png`, not `screenshot1.png`
4. **Add captions** - Update this README with descriptions

---

## Current Status

- **Screenshots:** ✅ Available - System state visualization with sample data
- **Videos:** Planned for future updates

Screenshots illustrate system state and outputs from the C++ trading platform.
